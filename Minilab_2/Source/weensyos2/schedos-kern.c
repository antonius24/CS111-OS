...
static pid_t shares[100];
static int run_cursor = 0;
static int total_share = 0;
...
start(void)
{
	...
	proc_array[0].p_priority = 1;	// for test
	proc_array[1].p_priority = 1;	// for test
	proc_array[2].p_priority = 1;	// for test
	proc_array[3].p_priority = 2;	// for test
	proc_array[4].p_priority = 4;	// for test
	...
	// Schedule after initialization
	//schedule();
	// Switch to the first process after intilization.	
	run(&proc_array[1]);
...
interrupt(registers_t *reg)
{
	...
	case INT_SYS_USER1: // use it as set priority system call
...
schedule(void)
{
	...
	else if (scheduling_algorithm == 1) // strictly priority algorithm
	{
	...
	else if (scheduling_algorithm == 2)  //separate-priority field
	{
		int i;
		int highest_priority;
		while (1)
		{
			highest_priority = NPROCS + 1;
			pid = (pid + 1) % NPROCS;
			for (i=0 ;i< NPROCS; i++)
			{	
	...
	else if (scheduling_algorithm == 3)  // proportional-share scheduling
	{
		...
		while (1)
		{
			...
			if (run_cursor == 0)
			{
				pid = (pid + 1) % NPROCS;
				total_share = 0;
				for (i=0 ;i< NPROCS; i++) // build the proportional shares
				{	
					if (proc_array[((pid+i) % NPROCS)].p_state == P_RUNNABLE)
					{
						for (j=0; j< (proc_array[(pid+i) % NPROCS].p_priority); j++)
						{
							shares[total_share+j] = (pid + i) % NPROCS;
						}
						total_share += proc_array[(pid+i) % NPROCS].p_priority;	
			...
			if ((proc_array[shares[run_cursor]].p_state == P_RUNNABLE))
			{
				run_cursor = (run_cursor + 1) % total_share;
				run(&proc_array[shares[(run_cursor - 1) % total_share]]);
			}
			run_cursor = (run_cursor + 1) % total_share;										
		}
