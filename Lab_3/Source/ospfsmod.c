...
ospfs_dir_readdir(struct file *filp, void *dirent, filldir_t filldir)
{
        ...
        while (r == 0 && ok_so_far >= 0 && f_pos >= 2) {
			...
			else
			{
                ...
                else // blank directory entries
                {
                        f_pos++;
                }

                if (f_pos >= dir_oi->oi_size) // end of directory
                {
                        r = 1;
                }
        }
...
{
        /* EXERCISE: Your code here */
        uint32_t freemap_start = OSPFS_FREEMAP_BLK;
        uint32_t freemap_end = ospfs_super->os_firstinob - 1;
        int i;

        //eprintk("In allocate_block\n");
        for (i=freemap_start; i <= freemap_end; i++)
        {
                void *block_ptr = ospfs_block(i);
                int block_offset = (i-freemap_start) * OSPFS_BLKSIZE*8;
                int j;
                for (j=0; j<OSPFS_BLKSIZE*8; j++)
                {
                        if ((block_offset+j) > ospfs_super->os_nblocks) // exceed the maximum num of blocks
                                break;
                        if (bitvector_test(block_ptr, j) == 1) // free 
                        {
                                bitvector_clear(block_ptr, j);
                                return block_offset+j;
                        }
...
free_block(uint32_t blockno)
{
        ...
        uint32_t freemap_end = ospfs_super->os_firstinob - 1;
        uint32_t data_start;
        int inodes_block_num = (ospfs_super->os_ninodes / OSPFS_BLKINODES);
        int i;

        //eprintk("In free_block\n");
        if ((ospfs_super->os_ninodes % OSPFS_BLKINODES) != 0) // add one more  block if not integer times
        {
                inodes_block_num++;
        }
        data_start = freemap_start + 1 + inodes_block_num; // first block after inodes
        if (blockno < data_start)
        {
                printk("reserved blocks should not be free!\n");
                return;
        }
        ...
        for (i=freemap_start; i <= freemap_end; i++)
        {
                ...
                for (j=0; j<OSPFS_BLKSIZE*8; j++)
                {
                        if ((block_offset+j) > ospfs_super->os_nblocks) // exceed the maximum num of blocks
                                break;
                        if ((block_offset+j) == blockno) // find the bitmap for the block 
                        {
                                bitvector_set(block_ptr, j);
                        }
...
{
        // Your code here.
        if (b < (OSPFS_NDIRECT + OSPFS_NINDIRECT)) // direct & indirect block
                return -1;
...
{
        // Your code here.
        if (b < OSPFS_NDIRECT)  // direct block
                return -1;
        else if (b < (OSPFS_NDIRECT + OSPFS_NINDIRECT)) // indirect block
                return 0;
        else    // doubly indirect block
...
add_block(ospfs_inode_t *oi)
{
        ...
        if (n < OSPFS_NDIRECT) // direct block
        {
                uint32_t new_index = allocate_block(); // index of the new block
                uint32_t dir_index = direct_index(n); // index of the block in the direct array
                if (new_index == 0)
                {
                        return -ENOSPC;
                }
                memset(ospfs_block(new_index), 0, OSPFS_BLKSIZE); // zero out
                oi->oi_direct[dir_index] = new_index; // store the new block index
                oi->oi_size = (n+1) * OSPFS_BLKSIZE;
        }

        else if (n < (OSPFS_NDIRECT + OSPFS_NINDIRECT)) // indirect block
        {
                uint32_t new_indirect_index = 0; // indirect block index
                uint32_t new_index = 0;
                uint32_t *indirect_block = 0;

                oi->oi_size = n * OSPFS_BLKSIZE;
                if ((n == OSPFS_NDIRECT) || (oi->oi_indirect == 0)) // add new indirect block
                {
                        new_indirect_index = allocate_block();
                        if (new_indirect_index == 0)
                                return -ENOSPC;
                        memset(ospfs_block(new_indirect_index), 0, OSPFS_BLKSIZE);
                        oi->oi_indirect = new_indirect_index;
                        allocated[0] = ospfs_block(new_indirect_index);
                        oi->oi_size += OSPFS_BLKSIZE;
                }
                new_index = allocate_block(); // new block stored in indirect block
                if (new_index == 0)
                {
                        if (allocated[0] != 0)
                        {
                                free_block(new_indirect_index);
                                oi->oi_indirect = 0;
                                return -ENOSPC;
                        }
                }
                memset(ospfs_block(new_index), 0, OSPFS_BLKSIZE);
                indirect_block = ospfs_block(oi->oi_indirect);
                indirect_block[direct_index(n)] = new_index;
                oi->oi_size += OSPFS_BLKSIZE;

        }

        else if (indir2_index(n) == 0)// doubly indirect block
        {
                uint32_t new_double_indirect_index = 0;
                uint32_t new_indirect_index = 0;
                uint32_t new_index = 0;
                uint32_t *indirect2_block = 0;
                uint32_t *indirect_block = 0;

                oi->oi_size = n * OSPFS_BLKSIZE;
                if ((n == OSPFS_NDIRECT + OSPFS_NINDIRECT) || (oi->oi_indirect2 == 0)) // add new doubly indirect block
                {
                        new_double_indirect_index = allocate_block();
                        if (new_double_indirect_index == 0)
                                return -ENOSPC;
                        memset(ospfs_block(new_double_indirect_index), 0, OSPFS_BLKSIZE);
                        oi->oi_indirect2 = new_double_indirect_index;
                        allocated[0] = ospfs_block(new_double_indirect_index);
                        oi->oi_size += OSPFS_BLKSIZE;
                }

                if (direct_index(n) == 0) // add new indirect block
                {
                        new_indirect_index = allocate_block();
                        if (new_indirect_index == 0)
                        {
                                if (allocated[0] != 0)
                                {
                                        free_block(new_double_indirect_index);
                                        oi->oi_indirect2 = 0;                    
                                        return -ENOSPC;
                                }
                        ...
                        indirect2_block[indir_index(n)] = new_indirect_index;
                        if (allocated[0] != 0)
                        {
                                allocated[1] = ospfs_block(new_indirect_index);
                        }
                        else
                        {
                                allocated[0] = ospfs_block(new_indirect_index);
                ...
                new_index = allocate_block();   // add new data block
                if (new_index == 0)
                {
                        if (allocated[1] != 0)
                        {
                ...
                memset(ospfs_block(new_index), 0, OSPFS_BLKSIZE);
                indirect2_block = ospfs_block(oi->oi_indirect2);
                indirect_block = ospfs_block(indirect2_block[indir_index(n)]);
                indirect_block[direct_index(n)] = new_index;
                oi->oi_size += OSPFS_BLKSIZE;
        }
	else
...
remove_block(ospfs_inode_t *oi)
{
        ...
        else if (n < (OSPFS_NDIRECT + OSPFS_NINDIRECT)) // indirect block
        {
                ...
                oi->oi_size = n * OSPFS_BLKSIZE;
                indirect_block = ospfs_block(oi->oi_indirect);
                if (indirect_block[direct_index(n)] != 0)
                {
                ...
                if (n == OSPFS_NDIRECT) // add new indirect block
                {
        ...
        else if (indir2_index(n) == 0)// doubly indirect block
        {
                ...
                if ((n == OSPFS_NDIRECT + OSPFS_NINDIRECT)) // remove doubly indirect block
                {
...
ospfs_read(struct file *filp, char __user *buffer, size_t count, loff_t *f_pos)
{
	...
	if (*f_pos + count > oi->oi_size) count = oi->oi_size - *f_pos;
...
ospfs_write(struct file *filp, const char __user *buffer, size_t count, loff_t *f_pos)
{
	...
	/* EXERCISE: Your code here */
	if ( *f_pos+count > oi->oi_size ) change_size(oi,*f_pos+count);
...
create_blank_direntry(ospfs_inode_t *dir_oi)
{
	...
	// going through the list of direntry to find an empty one
	// return the entry if found
	for ( offset = 0; offset < dir_oi->oi_size;
		offset += OSPFS_DIRENTRY_SIZE ) {
		entry = ospfs_inode_data(dir_oi,offset);
	//	eprintk("od->ino: %d\n", entry->od_ino);
		if ( entry->od_ino == 0 ) return entry;
	}

	 //empty entry not found, add block
	if (change_size(dir_oi, (dir_oi->oi_size + OSPFS_BLKSIZE)) != 0)
		return ERR_PTR(-ENOSPC);
	//if ( add_block(dir_oi) < 0 ) return ERR_PTR(-ENOSPC);
	else {
		entry = ospfs_inode_data(dir_oi,offset);
		return entry;
	}
...
ospfs_link(struct dentry *src_dentry, struct inode *dir, struct dentry *dst_dentry) {
        ...
        oi = ospfs_block(src_dentry->d_inode->i_ino);
        oi-> oi_nlink++;
        return 0;
}
...
{
	ospfs_inode_t *dir_oi = ospfs_inode(dir->i_ino);
	ospfs_direntry_t *empty_d;
	ospfs_inode_t *empty_i;
	...
	empty_d = create_blank_direntry(dir_oi);
	//eprintk("return from creat_blank\n");
	if ( IS_ERR(empty_d) ) return PTR_ERR(empty_d);
	//eprintk("return from creat_blank_2\n");
	memcpy( empty_d->od_name, dentry->d_name.name, dentry->d_name.len );

	for ( entry_ino = ospfs_super->os_firstinob; entry_ino < ospfs_super->os_ninodes; entry_ino++){
		empty_i = ospfs_inode(entry_ino);
		if ( empty_i->oi_nlink == 0 ) {
			empty_i->oi_nlink++;
			break;
		}
	}

	empty_d->od_ino = entry_ino;

	empty_i->oi_size = 0;
	empty_i->oi_ftype = OSPFS_FTYPE_REG;
	empty_i->oi_mode = mode;
	for ( ; i < OSPFS_NDIRECT; i++ ) empty_i->oi_direct[i] = 0;
	empty_i->oi_indirect = 0;
	empty_i->oi_indirect2 = 0;
...
ospfs_symlink(struct inode *dir, struct dentry *dentry, const char *symname)
{
        ospfs_inode_t *dir_oi = ospfs_inode(dir->i_ino);
        uint32_t entry_ino = 0;
        ospfs_direntry_t *direntry_ptr;
        ospfs_symlink_inode_t *sym_ptr;
        ospfs_inode_t *oi;
        uint32_t i;
        ...
        direntry_ptr = create_blank_direntry(dir_oi);
        ...
        sym_ptr = (ospfs_symlink_inode_t *)ospfs_inode(entry_ino);
        sym_ptr->oi_ftype = OSPFS_FTYPE_SYMLINK;
        sym_ptr->oi_nlink = 1;
        sym_ptr->oi_size = dentry->d_name.len;
        strcpy(sym_ptr->oi_symlink, symname);

        direntry_ptr->od_ino = entry_ino;
        strcpy(direntry_ptr->od_name, dentry->d_name.name);
...
ospfs_follow_link(struct dentry *dentry, struct nameidata *nd)
{
        ...
        char *char_ptr;
        size_t length;
        char destination[OSPFS_MAXNAMELEN];

        // Exercise: Your code here.
        //eprintk("In ospfs_follow_link\n");
        if (uid == 0) // root process
        {
                char_ptr = strchr(symlink, '?');
                char_ptr++;
                length = strcspn(char_ptr, ':');
                strncpy(destination, char_ptr, length);
