...
command_t ParsePipe(token_list list_head)
{
 ...
 //  sublist_first first_end sublist_second second_end
    else
    {
      ...
      subcommand_second = NULL;
      command = BuildPipe(subcommand_first, subcommand_second);
      //printf("TYPE: %d", command->u.command[0]->type);
      return command;      
    }
  }
  return command;
...
command_t
read_command_stream (command_stream_t s, int time_travel)
{
  command_t temp;
  //printf("IN READ\n");
  if (time_travel == 0)
  ...
  else  // enable time travel, return the whole sequence tree
  {
    if (s->is_time_travel == 0)
    {
      s->is_time_travel = 1;
      return s->complete_command;
    }
    else
    {
      s->is_time_travel = 0;
