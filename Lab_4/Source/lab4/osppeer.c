...
#define MAXFILESIZE	10*1024*1024  // Max size of the file: 10MB
...
static size_t read_tracker_response(task_t *t)
{
	...
	while (1) {
		...
		if (ret == TBUF_ERROR)
			...
			return split_pos;  // do not output error, just return 
	}
}
...
task_t *start_download(task_t *tracker_task, const char *filename)
{
	...
	size_t resized_buffer_used;
	char *resized_buffer = malloc(TASKBUFSIZ);
	size_t resized_buffer_size = TASKBUFSIZ;
	messagepos = -1;
	resized_buffer[0] = '\0';
	...
	while (1)	
	{
		...
		//osp2p_writef(tracker_task->peer_fd, "WANT %s\n", filename);
		//messagepos = read_tracker_response(tracker_task);
		resized_buffer_used = strlen(resized_buffer);		
		size_t resized_buffer_current = strlen(tracker_task->buf) + resized_buffer_used;
		if (resized_buffer_current > resized_buffer_size)
		{
			resized_buffer_size += TASKBUFSIZ;
			resized_buffer = realloc(resized_buffer, resized_buffer_size);
		}
		strncat(resized_buffer, tracker_task->buf, strlen(tracker_task->buf));
		if ((int)messagepos >= 0)
			break;	
	}
	messagepos += resized_buffer_used;
	if (resized_buffer[messagepos] != '2') {
		error("* Tracker error message while requesting '%s':\n%s",
		      filename, &resized_buffer[messagepos]);
		goto exit;
	}
	...
	if (s1 != resized_buffer + messagepos)
...
* Try 'rm %s.~*~' to remove them.\n", t->filename, t->filename);
	...
	while (1) {
		int ret = read_to_taskbuf(t->peer_fd, t);
		if (t->total_written > MAXFILESIZE)	// aviod buffer overrun 1
		{
			error("* Exceed max file size");
...
static void task_upload(task_t *t)
{
	...
	assert(t->head == 0);
	if ((t->tail - 12) > FILENAMESIZ)	// tail - (GET  OSP2P)
	{
	...
	// For task 2, check whether the peer is visiting the file not in the current directory.
	char *delims = "/";
	char *file_dir = NULL;
	file_dir = strtok(t->filename, delims);
	if (t->filename[0] == '/' ||	
		strcmp(file_dir, "..") == 0 ||
		strcmp(file_dir, "~"))	// tend to visit other directory
	{
		error("* Cannot visit files not in current directory %s", t->filename);
		goto exit;
	}
	
	t->disk_fd = open(t->filename, O_RDONLY); // open upload file
	if (t->disk_fd == -1) {
